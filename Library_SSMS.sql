...
alter procedure TheLostTribe(@BookTitle varchar(40),@BranchName varchar(40))
	as
	begin
...
alter function Books(@BookTitle varchar(40))
returns table
as
return (
...
/* #4- For each book that is loaned out from the "Sharpstown" branch and whose DueDate is today, retrieve the book title, the borrower's name, 
...
sp_help 'tbl_book_loans'

create function Books4(@BranchName varchar(40),@DueDate date)
	returns table
	as
...
create function Checkedout(@noofbooks int)
returns table
as
...
create procedure BookAuthored(@AuthorName varchar(45),@BranchName varchar(30))
as
begin
